...
export function deriveConnectionState(health: Awaited<ReturnType<typeof getConnectionHealth>>) {
  // Webhook registration is an internal operational detail, not something a
  // merchant can act on — it is retried automatically on every authenticated
  // request. Surfacing it as "Store connection needs attention" made a working
  // store look broken. `webhookCoverageReady` remains the internal signal.
  if (health.healthy) {
    return {
